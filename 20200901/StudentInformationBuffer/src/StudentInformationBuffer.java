...
将学生信息存入文件
 ...
 再次点击该按钮，不会再选择保存位置
 第二个显示一个txt文件的信息，没有去考虑txt文件中录入的不是学生信息这种情况，会将txt文件中所有信息输出
 ******************************************/
import javax.swing.*;
import java.awt.*;
                    ...
                    while (true) {
                        try {
                            if ((s = bufferedReader.readLine())!=null) {
                                jTextAreaStudent.append(s + System.lineSeparator());                                    //缓冲流读取
                            }
                            else{
...
class Student{
    ...
    void studentWrite(String Location){
        try {
            ...
            bufferedWriter.write(Name + " " + Collage + " " + Year + " " + Grade);                                  //缓冲流写入
