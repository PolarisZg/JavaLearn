...
class LessonFunction{
    ...
    void LessonAdd(){
        ...
        lesson.addElement(new Lesson(lessonName,lessonIsRequired,lessonTeacher,lessonTime,lessonScore));
        LessonNum++;
    }
    int LessonSearchName(String Name, int Start){
        for(int i = Start;i < LessonNum;i++){
            if(Name.equals(((Lesson)lesson.elementAt(i)).LessonName))
                return i;
        }
        return -1;
    }
    int LessonSearchIsRequired(Boolean required,int Start){
        for (int i = Start; i < LessonNum; i++) {
            if (((Lesson)lesson.elementAt(i)).LessonIsRequired == required){
                return i;
            }
        }
        return -1;
    ...
    int LessonSearchTeacherName(String teacherName, int Start){
        for(int i = Start;i < LessonNum;i++){
            if(teacherName.equals(((Lesson)lesson.elementAt(i)).LessonTeacher))
                return i;
        }
        return -1;
    }
            ...
            case 1:
                ...
                for(int i = 0;i < LessonNum && i != -1;){
                    if((i = LessonSearchName(name , i )) != -1) {
                        ((Lesson)lesson.elementAt(i)).PrintOut();
                        i++;
                    }
                }
    ...
    void LessonDeleteLocation(int location){
        lesson.removeElementAt(location);
        LessonNum--;
    }
    void LessonDelete(){
        System.out.print("输入课程名");
        ...
        ((Lesson)lesson.elementAt(deleteLocation)).PrintOut();
...
public class LessonManageVector {
